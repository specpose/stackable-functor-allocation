...
int main() {
    ...
    std::get<0>(nodes).copy_to_host(intermediateBuffer1.data());
    std::cout << "Jammed1: " << intermediateBuffer1 << std::endl;
    std::get<0>(nodes) = std::move(intermediateBuffer1);
    std::get<1>(edges).forward(std::get<0>(nodes), std::get<1>(nodes));
    std::vector<data_type> endBuffer1(11);
    std::get<1>(nodes).copy_to_host(endBuffer1.data());
    ...
    std::get<0>(nodes).copy_to_host(intermediateBuffer2.data());
    std::cout << "Jammed2: " << intermediateBuffer2 << std::endl;
    std::get<0>(nodes) = std::move(intermediateBuffer2);
    std::get<0>(edges).inverse(inputData, std::get<0>(nodes));
    std::vector<data_type> outputBuffer(12);
    inputData.copy_to_host(outputBuffer.data());
