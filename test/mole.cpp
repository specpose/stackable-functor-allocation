...
template<typename container_t> struct Invertible {
    Invertible() {}
    void forward(container_t& input, container_t& output) = delete;
    void inverse(container_t& input, container_t& output) = delete;
...
template<typename container_t> struct Adjacent_differences : public Invertible<container_t> {
    ...
    void forward(container_t& input, container_t& output) {
        first_value = input.at(0);
        for (std::size_t i = 1; i < input.size(); i++)
            output.at(i - 1) = input.at(i) - input.at(i - 1);
    }
    void inverse(container_t& input, container_t& output) {
